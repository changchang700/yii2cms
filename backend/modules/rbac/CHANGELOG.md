...
- Enh: Custom side menu via `rbac\Module::items` (mdmunir).
...
- Chg: Can customize return of `rbac\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `rbac\components\BizRule` (mdmunir).
...
- Chg: Remove `rbac\Module::allowActions`. Set access control directly with `rbac\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `rbac\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
